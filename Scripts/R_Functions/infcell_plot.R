...
#     inf.max <- round(max(int.pts0$z, na.rm=TRUE)+1000, 0)
    ...
    lapply(seq(nrow(unq.parms)), function(row){
        png(paste0('./Output/figures/land_parcombos_infcells',row,'.png'), width=800, height=600)
        ...
        filled.contour(x=int.pts$x, y=int.pts$y, z=int.pts$z, nlevels=20, xlim=c(xmin, xmax), ylim=c(ymin, ymax), zlim=c(inf.min, inf.max), plot.axes={
            points(y ~ x, pch=10)
        ...
        mtext('Max infected cells', side=3, outer=FALSE, line=2, font=2)
