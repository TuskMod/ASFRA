...
ridge.lasso <- function(rslt, rslt1, tmtab3, variables, mv.params){
    ...
    glmdat.est           <- model.matrix(~.,rslt  [,.SD,      .SDcols=!c('v','l','r','est','sounder.weeks','prop.infd','max.inc','tm.esc','inf.spd')])[,-1]
    glmdat.sounder.weeks <- model.matrix(~.,rslt1 [,.SD,      .SDcols=!c('v','l','r','sounder.weeks','est','prop.infd','max.inc','tm.esc','inf.spd')])[,-1]
    glmdat.prop.infd     <- model.matrix(~.,rslt1 [,.SD,      .SDcols=!c('v','l','r','prop.infd','sounder.weeks','est','max.inc','tm.esc','inf.spd')])[,-1]
    glmdat.max.inc       <- model.matrix(~.,rslt1 [,.SD,      .SDcols=!c('v','l','r','max.inc','sounder.weeks','prop.infd','est','tm.esc','inf.spd')])[,-1]
    glmdat.tm.esc        <- model.matrix(~.,tmtab3[exc==1,.SD,.SDcols=!c('v','l','r','tm.esc')])[,-1]
    glmdat.inf.spd       <- model.matrix(~.,rslt1 [,.SD,      .SDcols=!c('v','l','r','inf.spd','sounder.weeks','prop.infd','max.inc','tm.esc','est')])[,-1]
    ...
    ridge.mod.tm.esc        <-    glmnet(glmdat.tm.esc,        as.numeric(tmtab3[exc==1,tm.esc]), alpha=0, lambda=grid, family=quasipoisson())
    lasso.mod.tm.esc        <-    glmnet(glmdat.tm.esc,        as.numeric(tmtab3[exc==1,tm.esc]), alpha=1, lambda=grid, family=quasipoisson())
    ...
    cv.est.lasso            <- cv.glmnet(glmdat.est,           as.numeric(rslt[,est]),            alpha=1, lambda=grid, foldid=rslt[,lno],   family='binomial')
    ...
    cv.sounder.weeks.lasso  <- cv.glmnet(glmdat.sounder.weeks, as.numeric(rslt1[,sounder.weeks]), alpha=1, lambda=grid, foldid=rslt1[,lno],  family=quasipoisson(), control=list(maxit=999999))
    ...
    cv.prop.infd.lasso      <- cv.glmnet(glmdat.prop.infd,     as.numeric(rslt1[,prop.infd]),     alpha=1, lambda=grid, foldid=rslt1[,lno],  family=quasibinomial())
    ...
    cv.max.inc.lasso        <- cv.glmnet(glmdat.max.inc,       as.numeric(rslt1[,max.inc]),       alpha=1, lambda=grid, foldid=rslt1[,lno],  family=quasipoisson())
    ...
    cv.tm.esc.lasso         <- cv.glmnet(glmdat.tm.esc,        as.numeric(tmtab3[,tm.esc]),       alpha=1, lambda=grid, foldid=tmtab3[,lno], family=quasipoisson())
    cv.inf.spd.ridge        <- cv.glmnet(glmdat.inf.spd,       as.numeric(rslt1[,inf.spd]),       alpha=0, lambda=grid, foldid=rslt1[,lno])
    cv.inf.spd.lasso        <- cv.glmnet(glmdat.inf.spd,       as.numeric(rslt1[,inf.spd]),       alpha=1, lambda=grid, foldid=rslt1[,lno])
    ...
    coef.est.ridge           <- predict(ridge.mod.est,           s=cv.est.ridge$lambda.min,           type='coefficients')
    coef.est.lasso           <- predict(lasso.mod.est,           s=cv.est.lasso$lambda.min,           type='coefficients')
    coef.sounder.weeks.ridge <- predict(ridge.mod.sounder.weeks, s=cv.sounder.weeks.ridge$lambda.min, type='coefficients')
    coef.sounder.weeks.lasso <- predict(lasso.mod.sounder.weeks, s=cv.sounder.weeks.lasso$lambda.min, type='coefficients')
    coef.prop.infd.ridge     <- predict(ridge.mod.prop.infd,     s=cv.prop.infd.ridge$lambda.min,     type='coefficients')
    coef.prop.infd.lasso     <- predict(lasso.mod.prop.infd,     s=cv.prop.infd.lasso$lambda.min,     type='coefficients')
    coef.max.inc.ridge       <- predict(ridge.mod.max.inc,       s=cv.max.inc.ridge$lambda.min,       type='coefficients')
    coef.max.inc.lasso       <- predict(lasso.mod.max.inc,       s=cv.max.inc.lasso$lambda.min,       type='coefficients')
    coef.tm.esc.ridge        <- predict(ridge.mod.tm.esc,        s=cv.tm.esc.ridge$lambda.min,        type='coefficients')
    coef.tm.esc.lasso        <- predict(lasso.mod.tm.esc,        s=cv.tm.esc.lasso$lambda.min,        type='coefficients')
    coef.inf.spd.ridge       <- predict(ridge.mod.inf.spd,       s=cv.inf.spd.ridge$lambda.min,       type='coefficients')
    coef.inf.spd.lasso       <- predict(lasso.mod.inf.spd,       s=cv.inf.spd.lasso$lambda.min,       type='coefficients')

    coefs.out <- cbind(coef.est.ridge, coef.est.lasso
                        ,coef.sounder.weeks.ridge, coef.sounder.weeks.lasso
                        ,coef.prop.infd.ridge, coef.prop.infd.lasso
                        ,coef.max.inc.ridge, coef.max.inc.lasso
                        ,coef.tm.esc.ridge, coef.tm.esc.lasso
                        ,coef.inf.spd.ridge, coef.inf.spd.lasso)
    coefs.out <- data.table(as.matrix(coefs.out))
    s.vals <- unlist(lapply(colnames(coefs.out), function(x) tstrsplit(x, '=', keep=2)))
    coefs.out[, rwnames := rownames(coef.est.ridge)]
...
#     coefs.out <- rbind(coefs.out, unlist(list(colnames(coefs.out), 's'))
    setnames(coefs.out, c('est.ridge','est.lasso',
                          'sounder.weeks.ridge','sounder.weeks.lasso',
                          'prop.infd.ridge','prop.infd.lasso',
                          'max.inc.ridge','max.inc.lasso',
                          'tm.esc.ridge','tm.esc.lasso',
                          'inf.spd.ridge','inf.spd.lasso','rwnames'))
