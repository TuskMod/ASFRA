...
VisualOutputs <- function(lvtable, variables, land_grid_list, parameters, lands_names){
    ## NEED TO:
...
#             par(pdef2)
#         }
#         mcmapply(seirczbb.temporal, v=tm.pop.unq[land==l, var], l=tm.pop.unq[land==l,land], plt.i=seq(nrow(tm.pop.unq[land==l,])), MoreArgs = list(rows.plt=rows.plt, cols.plt=cols.plt))
...
#         png(paste0('./Output/figures/landscape_',lnd,'.png'))
...
#     mcmapply(function(v, l){
#         input.dat <- unq.eic[var==v & land==l & max.time >= 10,]
#         if(nrow(input.dat) != 0) {
# #         dev.new()
# #     browser()
#         usable.reps <- unique(input.dat[,rep])
#         png(paste0('./Output/figures/incidence_v', v, 'l', l, '.png'), width=1000, height=500*length(usable.reps))
...
#         #         dev.new()
#             usable.reps <- unique(input.dat[,rep])
#             png(paste0('./Output/figures/detection_v', v, 'l', l, '.png'), width=1000, height=500*length(usable.reps))
...
# #             }, movie.name = out.gif, ani.width=850, ani.height=600, interval=0.1, imgdir='./Output/figures')
...
#         if(v==1 & l != 1) {
#             dev.off()
#             png(paste0('./Output/figures/inc.time_land_',l,'.png'), width=pltdim[1]*450, height = pltdim[2]*500)
...
#         if(v==1 & l != 1) {
#             dev.off()
#             png(paste0('./Output/figures/inc.cum_land_',l,'.png'), width=pltdim[1]*450, height = pltdim[2]*500)
#             par(mfrow=pltdim, mar=c(2,2,0,0), oma=c(2,2,0.2,0.2))
#         } else if (v==1 & l == 1){
#             png(paste0('./Output/figures/inc.cum_land_',l,'.png'), width=pltdim[1]*450, height = pltdim[2]*500)
#             par(mfrow=pltdim, mar=c(2,2,0,0), oma=c(2,2,0.2,0.2))
...
#
#     png('./Output/figures/inc_spread.png', width=1200, height=1200)
#     par(mfrow=c(2,2))
...
#     png('./Output/figures/land_parcombos_maxdist.png', width=1000, height=800)
#     par(mfrow=c(length(unique(unq.parms[,density])), nrow(unq.parms)/length(unique(unq.parms[,density]))))
#     lapply(seq(nrow(unq.parms)), function(y){
#         sub.parms <- unq.parms[y,]
